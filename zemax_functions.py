...
Description: Container for various useful functions when scripting the ZOS_API
'''
##import packages
import os
import numpy as np
...
def get_footprint(api, system, config=1, field=0, wave=0, nrays=10, surface=0, delete_vignetted=True, outpath=".\\", outfile="footprint_res.txt"):
    """
    Computes footprint diagram data for given configuration, field #, and wavelength # using ZOS-API analysis.

    Parameters:
    - api: The ZOS API instance
    ...
    footprint = system.Analyses.New_Analysis(api.Analysis.AnalysisIDM.FootprintSettings) #Footprint Diagram not yet built in
    #modify settings
...
def get_spot_sizes(api, system, configs, fields, waves, nrays):
    """
    ...
    spot = system.Analyses.New_Analysis_SettingsFirst(api.Analysis.AnalysisIDM.StandardSpot)
    ...
    spot_setting.ReferTo = api.Analysis.Settings.Spot.Reference.Centroid
    spot_setting.Patterns = api.Analysis.Settings.Spot.Patterns.Dithered 
...
def make_spot_diagram(api, system, config, fields, waves, nrays, band, band_n, outpath=".\\", norm_waves=True):
    """
    ...
    max_rays = nrays
    normUnPolData = raytrace.CreateNormUnpol((max_rays + 1) * (max_rays + 1), api.Tools.RayTrace.RaysType.Real, nsur)
    ...
    if system.SystemData.Fields.GetFieldType() == api.SystemData.FieldType.Angle:
        field_type = 'Angle'
    elif system.SystemData.Fields.GetFieldType() == api.SystemData.FieldType.ObjectHeight:
        field_type = 'Height'
    elif system.SystemData.Fields.GetFieldType() == api.SystemData.FieldType.ParaxialImageHeight:
        field_type = 'Height'
    elif system.SystemData.Fields.GetFieldType() == api.SystemData.FieldType.RealImageHeight:
        field_type = 'Height'
    ...
    for field in fields:
        ...
        for wave in waves:
            ...
            for i in range(1, (max_rays + 1) * (max_rays + 1) + 1):
                ...
                while (px*px + py*py > 1):
                    py = np.random.random() * 2 - 1
                normUnPolData.AddRay(wave, hx_ar[field -1], hy_ar[field -1], px, py, Enum.Parse(api.Tools.RayTrace.OPDMode, "None"))
...
def local_to_global_coords(global_matrix, local_coords):
    '''
